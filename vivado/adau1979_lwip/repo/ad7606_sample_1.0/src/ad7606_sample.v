...
reg signed[31:0]   adc_buf_data ;  //write data for fifo
...
always @(posedge adc_clk or negedge adc_rst_n)
    if(!adc_rst_n) sample_trig_d0 <= 0;
    else           sample_trig_d0 <= ad7606_if_m0.clk_sample;  // Ð´Óò´òÒ»ÅÄ

always @(posedge M_AXIS_CLK or negedge M_AXIS_RSTN) begin
    if(!M_AXIS_RSTN) {trig_r2, trig_r1} <= 2'b00;
    else             {trig_r2, trig_r1} <= {trig_r1, sample_trig_d0};
end
...
xpm_fifo_async_inst (
   ...
   .wr_clk         (adc_clk),
   .wr_en          (ad_data_valid),
   .din            ({ad_ch8,ad_ch7,ad_ch6,ad_ch5,ad_ch4,ad_ch3,ad_ch2,ad_ch1}),
   .rd_clk         (M_AXIS_CLK),
   .rd_en          (adc_buf_rd),
   .dout           (M_AXIS_tdata),
   .empty          (empty),

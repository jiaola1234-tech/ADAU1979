...
always@(posedge clk or negedge rst_n) begin
        ...
        else if(clk_count>=499)
        begin 
        clk_count <= 0;
        clk_sample_reg <= !clk_sample_reg;
        end
        else
           clk_count <= clk_count + 1;
     end
assign clk_sample = clk_sample_reg;

reg  clk_sample_d;
    wire clk_sample_rise;

    always @(posedge clk or negedge rst_n)
        if (!rst_n) clk_sample_d <= 0;
        else        clk_sample_d <= clk_sample_reg;

    assign clk_sample_rise = clk_sample_reg && !clk_sample_d;   // 每500μs一个20ns脉冲
	...
	begin
		case(state)
			...
			begin
				...
				ad_convstab<=1'b1;
				i <= 6'd0;
//clk_sample1上升沿开始进行采样
				if(clk_sample_rise) begin
					state<=AD_CONV;
				end
				else
					i <= i + 6'd1;
